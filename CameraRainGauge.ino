...
void loop() 
{
  ...
  if ((WiFi.status() != WL_CONNECTED) || (WiFi.status() == WL_NO_SSID_AVAIL )  || (WiFi.status() == WL_CONNECTION_LOST) || brownout == 2)
  {
  ...
  if ((MINUTE == 59) && (SECOND == 59)) // one hour counter
  {
   ...
   if ((HOUR == 23) && (MINUTE == 57) && (SECOND == 0) && (DOW == 6))
   {
      ...
      listDel("/");  //removes files from Sunday thru Saturday.
...
void listDel(char * dir)
{
  ...
  if(a < 8)  
  {
    ...
    for(a = 1;a < 8; a++)  //Delete current week of files; keep from getting too many log files.
    {
