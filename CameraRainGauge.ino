...
//
//                       Developed AsyncWebServer 11/07/2019; modified with fileRead and not found function.  Adding event wifi log.
//
...
//
//                       Rain gauge code developed by Muhammad Haroon.  Thank you Muhammad.
//
...
void speak()
{
  ...
  char t_buffered1[14];
  dtostrf(temperature, 7, 2, t_buffered1);

  char t_buffered2[14];
  dtostrf(hum, 7, 2, t_buffered2);
...
void webInterface()
{
  ...
  char glat[10]; 
  dtostrf(gpslat, 9, 4, glat); 

  char glng[10];
  dtostrf(gpslng, 9, 4, glng);
  ...
  char humidity[7];
  dtostrf(hum, 6, 1, humidity); 

  char dewpoint[7]; 
  dtostrf(dew, 6, 1, dewpoint);

  char barometric[9];
  dtostrf(currentPressure, 8, 3, barometric); 

  char diff[9]; 
  dtostrf(difference, 8, 3, diff);

  char rain5[10]; 
  dtostrf(rain5min, 6, 3, rain5);

  char rain60[10];
  dtostrf(rainHour, 6, 3, rain60); 

  char rain24[10];
  dtostrf(rainDay, 6, 3, rain24); 

  char alt[10];
  dtostrf(altFeet, 6, 3, alt); 
